...
3. 通过 HTTP 调用业务服务公开的内部 command；
4. 按通用 retry/backoff、pause/resume 和 misfire policy 控制触发，并记录本次 dispatch 的成功或失败；
5. 在进程停止时优雅退出。
...
├── scheduler_test.go       # unit/contract tests
├── Dockerfile
└── docs/
```
...
- 启动顺序：解析 JSON → 注册全部 cron job → 任一 job 非法则进程退出 → 启动 cron；
...
测试覆盖：严格配置解析、未知字段拒绝、HTTP method/body、retry policy、pause/resume、request_id/idempotency headers、30 秒 client timeout、job header、2xx/非 2xx 结果及 cron service 注册。
