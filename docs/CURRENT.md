...
更新日期：2026-09-03。本文只描述当前仓库实现；历史报告、设计愿景和未来工作不构成完成证据。
...
| Dispatch | `POST` / `PUT` JSON command，30 秒总超时，携带 occurrence、request、idempotency、trace headers |
...
3. `SCHEDULER_JOBS_JSON` 和受信 command 可配置任意合法 `http` / `https` host；内部目标 allowlist 目前由部署
   与网络策略执行，进程内仅拒绝无 host、非 HTTP(S)、userinfo 和 fragment。
4. inbound authorization 当前是一个共享 Bearer service token，没有 caller identity 或细粒度 IAM permission。
5. 出站 client 当前设置 30 秒 overall timeout 并传播 context，但没有独立的 connect、TLS handshake 和
   response-header timeout budget。
6. `docs/SLO.md` 定义指标名与目标；本仓当前只有结构化 dispatch 日志，没有内置 metrics exporter、dashboard
   或 PrometheusRule，因此 SLO 不代表已实测达标。
7. Redis 集成测试依赖显式 `SCHEDULER_REDIS_TEST_URL`；未提供真实 Redis 时该用例会跳过，不能冒充集成通过。

## 本轮治理范围

- 标准文档入口、contract provenance 和 OpenAPI operation governance metadata；
- 不改变 Go runtime、API 行为、数据模型或部署依赖；
