...
| Dispatch | `POST` / `PUT` JSON command，30 秒总超时，携带 occurrence、request、idempotency、trace headers；默认拒绝特殊地址，`SCHEDULER_INTERNAL_TARGET_ALLOWLIST` 可严格覆盖精确 internal host/CIDR pair，dispatch 前单次解析并 pin 地址，禁跟随 redirect，response body 上限 1 MiB |
...
3. `SCHEDULER_JOBS_JSON` 和受信 command 仍可配置可路由的 global `http` / `https` host；显式 internal
   allowlist 只覆盖精确列出的 private/loopback/IPv6 ULA pair，进程内策略不替代部署的 hostname/CIDR
   allowlist、egress network policy 和目标服务认证。
