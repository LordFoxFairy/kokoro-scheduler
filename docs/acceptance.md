...
| 非法配置 | 未知字段、重复名称、非法 cron、retry 零值/越界值、最大 backoff 小于初始值 | 启动失败 |
...
| 重试退避 | 注入 Clock、Sleeper、随机序列 | 指数 ceiling 受 `max_backoff_seconds` 限制，每次使用 full jitter |
| 重试总窗口 | 注入 Clock 并推进到 deadline | deadline 到达后不启动下一次 attempt，返回最后一次 target 结果 |
| pause/resume | application Scheduler control | pause 跳过后续 occurrence，resume 恢复 |
| misfire | `Scheduler.Dispatch(ctx, job, scheduledAt, observedAt)` | `skip` 丢弃，`fire_once` 补发一次 |
